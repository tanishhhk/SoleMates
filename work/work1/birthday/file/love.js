(function(window){
    ...
    Seed.prototype = {
        ...
        drawText: function() {
            var ctx = this.tree.ctx, heart = this.heart;
            var point = heart.point, color = "red", 
                scale = heart.scale;
            ctx.save();
            ctx.strokeStyle = "black";
            ctx.fillStyle = "black";
            ctx.translate(point.x, 200);
            ctx.scale(scale, scale);
            ctx.moveTo(0, 0);
    	    ctx.lineTo(15, 15);
    	    ctx.lineTo(130, 15);
            ...
            ctx.fillText("Happy Birthday Nishtha :)", 30, -10);
            ctx.fillText("Click on the Heart <3", 28, 10);
            ctx.restore();
        },
        clear: function() {
            var ctx = this.tree.ctx, cirle = this.cirle;
